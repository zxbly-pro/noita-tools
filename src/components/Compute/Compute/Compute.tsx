    ...
    <Container className="py-4">
      <div className="mb-4">
        ...
        <p className="text-muted mb-0">
          此设备的计算能力可用于帮助其他设备上的搜索。连接到计算池以为启用集群的搜索贡献处理能力。
        </p>
      </div>
      ...
      <div className="border rounded p-3 mb-4">
        ...
        {computeInfo.jobName && (
          <div className="border-start border-4 border-primary ps-3 mb-3">
            <h6 className="mb-2">当前任务： {computeInfo.jobName.substring(0, 6)}</h6>
            <div className="text-muted small">
              <div>
                处理种子 {localizeNumber(computeInfo.chunkFrom)} - {localizeNumber(computeInfo.chunkTo)}
              </div>
              <div>已检查种子数： {localizeNumber(computeInfo.jobStats.checked)}</div>
              <div>
                剩余时间：{" "}
