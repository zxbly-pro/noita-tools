...
export class AlwaysCastInfoProvider extends InfoProvider {
  ...
  providePos(x: number, y: number) {
    ...
    const good_cards = ["DAMAGE", "CRITICAL_HIT", "HOMING", "SPEED", "ACID_TRAIL", "SINEWAVE"];
    ...
    if (r <= 50) {
      ...
      if (p <= 86) {
        card = this.randoms.GetRandomActionWithType(x, y, level, ACTION_TYPE.MODIFIER, 666);
      } else if (p <= 93) {
        card = this.randoms.GetRandomActionWithType(x, y, level, ACTION_TYPE.STATIC_PROJECTILE, 666);
      } else if (p < 100) {
        card = this.randoms.GetRandomActionWithType(x, y, level, ACTION_TYPE.PROJECTILE, 666);
      } else {
        card = this.randoms.GetRandomActionWithType(x, y, level, ACTION_TYPE.UTILITY, 666);
      }
    }
