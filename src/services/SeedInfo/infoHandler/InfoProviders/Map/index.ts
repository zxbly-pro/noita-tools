...
import { NIGHTMARE_WORLD_MAP_DATA_URI } from "./nightmareWorldMap";
...
export class MapInfoProvider extends InfoProvider {
  ...
  constructor(randoms: IRandom, isNightmare = false) {
    super(randoms);
  ...
  async loadWorldMap() {
    ...
    const worldMapDataUri = this.isNightmare
      ? NIGHTMARE_WORLD_MAP_DATA_URI
  ...
  setNightmareMode(isNightmare: boolean) {
    if (this.isNightmare === isNightmare) {
